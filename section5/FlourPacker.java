...
public class FlourPacker {
  ...
  public static boolean canPack(int bigCount, int smallCount, int goal) {
    ...
    // at this point sum will be greater than or equal to goal
    if (bigCount * 5 + smallCount == goal) {
      return true;
    } else {
      // at this point the total sum is greater then goal. Start with all the bigCount
      // bags and see if goal divided by their total equals a remainder that can be
      // made up by total smallCount, otherwise decrement bigCount
      int totalBigCount = bigCount * 5;
      while (totalBigCount >= 0) {
        if (totalBigCount == 0) {
          return smallCount >= goal ? true : false;
        } else if (goal % totalBigCount <= smallCount) {
          return true;
        } else {
          totalBigCount -= 5;
        }
